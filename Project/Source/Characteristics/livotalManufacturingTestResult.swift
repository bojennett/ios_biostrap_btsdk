...
class livotalManufacturingTestResult: Codable {
	...
	init(_ data: Data) {
		...
		let temp_data			= (data[0] >> 2) & 0x03
		let flash_if_data		= (data[0] >> 4) & 0x03
		let flash_array_data	= (data[0] >> 6) & 0x03
		let ppg_if_data			= (data[1] >> 0) & 0x03
		let ppg_fifo_data		= (data[1] >> 2) & 0x03
		let imu_if_data			= (data[1] >> 4) & 0x03
		let imu_fifo_data		= (data[1] >> 6) & 0x03
		let led_data			= (data[2] >> 0) & 0x03
